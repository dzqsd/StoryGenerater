...
### 策划阶段（6 大 AI 对话模块）
- **总策划** — 全能顾问，可同时提取角色、剧情、章节等所有结构化数据
- **世界观** — 设计故事背景、题材、设定体系，确认后自动保存
- **人物** — 创建角色档案（姓名、身份、性格、背景），AI 辅助丰富细节
- **剧情** — 主线概要规划，自动识别伏笔、冲突、角色弧光
- **章节** — 章节划分与大纲生成，支持可视化时间线
- **修订** — 对已完成章节进行讨论和修改建议
...
AI 回复中的结构化数据（角色信息、剧情概要、章节结构）会自动解析并保存到数据库。
